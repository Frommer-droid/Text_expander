...
class UiSetupMixin:
    ...
    def _create_widgets(self):
        ...
        # objectName не задаём: используется нейтральная роль обязательной темы.
    ...
    def _icon_buttons(self):
        """Кнопки с пиктограммами для enforce_button_proportions."""
        names = (
            "capture_window_button",
            "new_category_button",
            "new_snippet_button",
            "rename_button",
            "delete_button",
            "save_button",
            "export_snippets_button",
            "import_snippets_button",
            "export_settings_button",
            "import_settings_button",
            "export_all_data_button",
            "import_all_data_button",
        )
        buttons = []
        for name in names:
            button = getattr(self, name, None)
            if button is not None and not button.icon().isNull():
                buttons.append(button)
        return buttons

    def _enforce_icon_button_proportions(self):
        enforce_button_proportions(self._icon_buttons())
    ...
    def _apply_styles(self, scale_factor=None):
        ...
        if app is not None:
            apply_theme(app, scale_factor=scale_factor)
        self._enforce_icon_button_proportions()
    ...
    def _apply_admin_status_style(self):
        if not hasattr(self, "admin_status_label"):
            return
        is_admin = bool(getattr(self, "_admin_status_is_admin", False))
        # Роль берётся из ядра темы: QLabel#healthGood/healthBad.
        self.admin_status_label.setObjectName(
            "healthGood" if is_admin else "healthBad"
        )
        # Инлайн-цвета запрещены: стиль полностью из глобальной темы.
